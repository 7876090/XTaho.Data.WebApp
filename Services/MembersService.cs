...
{ 
    ...
    {       
        ...
        public async Task<QueryResult<MemberModel>> AddMember(MemberModel member)
        {
            string qText = member.AddRecordQueryText<MemberModel>();
            return await Connector.AddRecordAsync<MemberModel>(qText, member);
        }

        public async Task<QueryResult<MemberModel>> UpdateMember(MemberModel member)
        {
            string qText = member.UpdateRecordQueryText<MemberModel>();
            return await Connector.UpdateRecordAsync<MemberModel>(qText, member);
        }
    }
}
